...
from .cirtorch.utils.whiten import pcawhitenlearn, whitenapply
...
def main(descriptors, output, num_matched,
    ...
    mask = np.full((len(query_names), len(db_names)), True, bool)
    if per_camera:
    ...
    # squeeze both. dims NxD
    db_desc = tensor_from_names(db_names).squeeze()
    query_desc = tensor_from_names(query_names).squeeze()
