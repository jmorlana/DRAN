...
from ..lib.geometry import Pose, Camera
from ..lib.datasets.view import read_image
from ..utils.data import Paths
from ..S2DHM.exhaustive_search import exhaustive_search, pack_to_s2dhm, get_camera, scale_to_image
from ..S2DHM.solve_pnp import solve_pnp
from ..S2DHM import s2dhm, keypoint_association, plot_correspondences
...
class BaseRefiner:
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, T_init: Pose,
        ...
        for image_scale in multiscales:
            ...
            for idx, dbid in enumerate(dbid_to_p3dids):
                ...
                features_ref_dense, scales_ref,_,_ = self.dense_feature_extraction(
            ...
            features_query, scales_query,_,_ = self.dense_feature_extraction(
    ...
    def refine_query_pose_rerank(self, qname: str, qcamera: Camera, T_init: Pose,
                          p3did_to_dbids: Dict[int, List],
                          multiscales: Optional[List[int]] = None) -> Dict:
        ...
        for image_scale in multiscales:
            ...
            features_query, scales_query, image_resized, query_hypercolumn = self.dense_feature_extraction(
                        image_query, qname, image_scale)
            ...
            for idx, dbid in enumerate(dbid_to_p3dids):
                ...
                random.seed(0)
                if self.conf.light:
                    selected = np.random.RandomState(0).choice(len(p3dids), 512, replace=False)
                    p3dids = p3dids[selected]

                features_ref_dense, scales_ref, image_ref, ref_hypercolumn = self.dense_feature_extraction(
                        images_ref[idx], rnames[idx], image_scale)
                dbid_p3did_to_feats[dbid] = self.interp_sparse_observations(
                        features_ref_dense, scales_ref, dbid, p3dids)

                width_r, height_r = ref_hypercolumn.shape[2:]
                sparse_ref_hypercolumn, ref2D_points = self.interp_sparse_hypercolumn(
                        ref_hypercolumn, scales_ref[1], dbid, p3dids) 

                del features_ref_dense, ref_hypercolumn
                
                #Exhaustive search
                sparse_ref_feats, dense_query_feats, width, height = pack_to_s2dhm(query_hypercolumn, sparse_ref_hypercolumn)
                matches_2D, mask = exhaustive_search(
                    dense_query_feats.cuda(),
                    sparse_ref_feats.cuda(),
                    image_resized.shape[-2:], 
                    [width, height], 
                    [8,8], # cell_size
                    self.conf.s2dhm_factor) # factor
                
                ref2D_points = scale_to_image(ref2D_points, image_ref.shape[-2:], [width_r, height_r])

                #PnP
                visible_ids = sparse_ref_hypercolumn.keys()
                p3d_pnp = np.stack([self.model3d.points3D[p3did].xyz for p3did in visible_ids])

                points_2D = np.reshape(
                    matches_2D.cpu().numpy()[mask], (-1, 1, 2))
                points_3D = np.reshape(
                    p3d_pnp[mask], (-1, 1, 3))
                distortion_coefficients, intrinsics = get_camera(qcamera, scales_query[0])
                
                prediction = solve_pnp(
                    points_2D=points_2D,
                    points_3D=points_3D,
                    ...
                    minimum_matches = 12,
                    minimum_inliers = 5,
                    reference_filename=str(self.paths.reference_images / rnames[idx]),
                    reference_2D_points=ref2D_points,
                    reference_keypoints=None,
                    ref_id = dbid)


                # If PnP failed, do not store prediction
            ...
            if len(predictions):
                query_image = str(self.paths.query_images / qname)
                export, best_prediction = s2dhm._choose_best_prediction(predictions, query_image)
                log_images = False
                if log_images:
                    if np.ndim(np.squeeze(best_prediction.query_inliers)):
                        s2dhm._plot_inliers(
                            ...
                            export_filename=s2dhm.output_converter(query_image))

                    plot_correspondences.plot_image_retrieval(
                        left_image_path=query_image,
                        right_image_path=best_prediction.reference_filename,
                        title='Best match',
                        export_folder='/home/jmorlana/experiments_reuse/outputs/S2DHM/retrieval_RC/',
                        export_filename=s2dhm.output_converter(query_image))

                    

                if self.conf.only_pnp:
                    ret = {'T_init': T_init}
                    T_opt = Pose.from_4x4mat(best_prediction.matrix)
                    dR, dt = (T_init.inv() @ T_opt).magnitude()
                    ret = {
                        **ret,
                        'success': True,
                    ...
                    T_init = Pose.from_4x4mat(best_prediction.matrix)
                    ref_id = best_prediction.ref_id

                    # Get features from the best candidate
                    p3did_to_feat = self.get_features_winner(
                            p3did_to_dbids, dbid_p3did_to_feats, ref_id)
            else:
                p3did_to_feat = self.aggregate_features(
                        p3did_to_dbids, dbid_p3did_to_feats)
    ...
    def dense_feature_extraction(self, image: np.array, name: str,
        ...
        features, scales, weight, embedding, image_tensor, hypercolumn = self.feature_extractor(
                image, image_scale)
        self.log_dense(name=name, image=image, image_scale=image_scale,
                       features=features, scales=scales, weight=weight, embedding = embedding)
        ...
        return features, scales, image_tensor, hypercolumn
    ...
    def interp_sparse_hypercolumn(self,
                                   feature_maps: List[torch.Tensor],
                                   feature_scales: List[float],
                                   image_id: float,
                                   p3dids: List[int],
        ...
        return feature_dict, p2d_feat[valid]
