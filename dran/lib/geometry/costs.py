...
class DirectAbsoluteCost:
    ...
    def jacobian(
        ...
        J_p2D_p3D, _ = camera.J_world2image(p3D_q) # Valid is not used and eps may be too lowp3D_q
        ...
        if torch.isnan(J).any() or torch.isnan(J_p2D_T).any():
            print('Estimation of Jacobian gives NaN values')
            # Point i with z = 0
            # Leads to J_p2D_p3D[i] = inf
            # Then J_p2D_T becomes NaN
            # J is corrupted
